...
my_canvas.create_text(140,805,anchor="nw", text="Soft Skills:", font=FillFont)

#IC search fill (Must add command to work with button)
entryIC = Entry(root, width = 100)
my_canvas.create_window(500, 265, anchor="nw", window=entryIC)

#Search Button (Need to add command to search database)
...
#Edit fill (Must pull data from database and allow edit)
...
#Save Edit Button (Need to add function to save edit from form)
img_label = Label(image=SaveEditButtonImg)
ButtonSaveEdit = Button(root, image=SaveEditButtonImg, borderwidth=0, highlightthickness=0, bd=0)
my_canvas.create_window(140, 850, anchor="nw", window=ButtonSaveEdit)
